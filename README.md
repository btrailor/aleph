...
**New!** Complete local development build system:

- **⚡ Quick build:** `./build-bees.sh` - Simple one-command build
- **🛠️ Development:** `./build-dev.sh --verbose` - Enhanced build with options
- **🔍 Environment check:** `./check-build-env.sh` - Verify build setup
- **📖 Full documentation:** [BUILD_SYSTEM_README.md](BUILD_SYSTEM_README.md)

**Features:**

- Docker-based AVR32 toolchain (no local setup required)
- Local source file mounting for easy development
- Complete ASF integration with all drivers
- Builds all 76+ bees operators, pages, and scalers
- Generates ready-to-flash `.hex` files in `build-output/`

## �📦 SD Card Bundle Creation

**Complete SD card bundling system for easy deployment:**

- **📖 [SD Card Bundler Documentation](ALEPH_SDCARD_BUNDLER.md)** - Complete guide to creating SD card bundles
- **⚡ Quick start:** `utils/sdcard-bundler/quick-bundle.sh`
- **🎯 One command:** `./bundle-sdcard.sh 0.8.1` creates complete SD card image
