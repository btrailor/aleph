...
// the static mem ctlr before C runtime starts up and use external SDRAM for the
// heap.
...
void check_startup(void) {
	...
	(app_event_handlers)[e.type](e.data);
        ...
        if (launch) {
            ...
            if (firstrun) {
                // successfully launched on firstrun, so write magic number to
                // flash
